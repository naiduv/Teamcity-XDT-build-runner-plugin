...
import xdtRunner.agent.block.Block;
import xdtRunner.agent.block.MatchedBlock;
import xdtRunner.agent.block.XdtBlockList;
import xdtRunner.agent.line.Line;
import xdtRunner.agent.line.XdtLineListParser;
...
public class LogParser {
    ...
    LogParser(jetbrains.buildServer.agent.BuildProgressLogger logger, boolean warningsAsErrors, java.io.File lineListDefinition) {
        ...
        XdtLineListParser.ParseLines(lineListDefinition);

        for (Block block : XdtBlockList.blocks)
            block.init();
    }
    ...
    private void logLine(String message) {
        // Now check message
        for (Line line : XdtLineListParser.lines) {
